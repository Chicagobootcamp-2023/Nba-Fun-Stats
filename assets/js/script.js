    ...
    .then(function (giphs) {
      ...
      for (var i = 0; i < giphs.data.length; i++) {
        ...
        imageTag.setAttribute(
          "src",
          giphs.data[i].images.fixed_height_still.url
        );
        imageTag.setAttribute(
          "data-animate",
          giphs.data[i].images.fixed_height.url
        );
        imageTag.setAttribute(
          "data-still",
          giphs.data[i].images.fixed_height_still.url
...
document.addEventListener("DOMContentLoaded", function () {
  const generatedYouTube = document.querySelector(".youtube-section");
  const generatedGiphy = document.querySelector(".gif-section");

  searchButton.addEventListener("click", function () {
    generatedYouTube.style.display = "flex";
    generatedGiphy.style.display = "flex";
  });
});
